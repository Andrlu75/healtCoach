...
interface MiniAppWorkoutsProps {
  clientId: string;
  onWorkoutClick?: (workoutId: string, assignmentId: string) => void;
}

export const MiniAppWorkouts = ({ clientId, onWorkoutClick }: MiniAppWorkoutsProps) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="mt-3 text-gray-500 text-sm">Загрузка тренировок...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-4">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Назначенные программы</p>
      </div>
          ...
          >
            {pendingWorkouts.map((assignment) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => onWorkoutClick?.(assignment.workout.id, assignment.id)}
          ...
          >
            {completedWorkouts.map((assignment) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => onWorkoutClick?.(assignment.workout.id, assignment.id)}
                className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 opacity-75 cursor-pointer"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="font-medium text-gray-700">{assignment.workout.name}</h3>
      ...
      {assignments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <h3 className="text-lg font-medium text-gray-700">Нет тренировок</h3>
...
export default MiniAppWorkouts;
